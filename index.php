...
if (isset($_GET['phone_number']) && $_GET['agent']) {
	...
	mysql_query("DELETE FROM `cart` WHERE `phone`='".$_SESSION['phone_number']."'");
}
...
<body>
	...
	<script type="text/javascript">
		...
		function reload(){
			...
			$.ajax({
				...
				success: function(response){
					$('#myModal1').modal('hide');
					window.location.href="http://36.255.69.216/madchef/index.php?phone_number=<?php echo $_SESSION['phone_number'];?>&agent=<?php echo $_SESSION['agent'];?>&brand="+brand+"&branch="+branch;
				}
			});
		}
		...
		function add_to_cart(product_name, product_unit_price, extra_item) 
		{
			...
			console.log(extra_choice);
			var extra_choice_len=extra_choice.length;
			console.log(extra_choice_len);
			if(extra_choice_len == extra_item)
			{
				console.log("ok");
			}
			else
			...
			if(extra_choice_len == extra_item){
				...
				$("#alert_message").html("Please Select Subproduct Item...");
			}
