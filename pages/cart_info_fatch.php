...
if (isset($_POST['cart_identy']) && $_POST['cart_identy']=='cart_funtion') 
{
	...
	$extra_instruction = preg_replace("/[0-9\@\.\;\']+/",'', $_POST['extra_instruction']);
	...
	//check cart previous
	// $sql_cart_previous_check=mysql_query("SELECT * FROM `cart` WHERE `product_id`='$product_id'");
	// $cart_previous_result=mysql_fetch_assoc($sql_cart_previous_check);
	// $previous_cart_count=mysql_num_rows($sql_cart_previous_check);
	// if($previous_cart_count>0)
	// {
	// 	$new_product_quantity=$cart_previous_result['quantity']+1;
	// 	$new_final_sd=$new_product_quantity*($cart_previous_result['sd']/$cart_previous_result['quantity']);
	// 	$new_final_vat=$new_product_quantity*($cart_previous_result['vat']/$cart_previous_result['quantity']);
	// 	$new_total_product_price=$new_product_quantity*($cart_previous_result['total_price']/$cart_previous_result['quantity']);
	// 	$cart_update=mysql_query("UPDATE `cart` SET `quantity`='$new_product_quantity', `sd`='$new_final_sd',`vat`='$new_final_vat',`total_price`='$new_total_product_price' WHERE `product_id`='$product_id'");
	// }
	// else
	// {
	...
	echo "INSERT INTO `cart`(`phone`, `product`, `product_id`, `sub_product`, `adons_product`, `size`, `unit_price`, `quantity`, `discount`, `sd`, `vat`, `total_price`, `extra_instruction`, `product_not_available`) VALUES ('$phone','$product_name','$product_id','$sub_product','$adons_product_id','','$product_unit_price','$product_quantity','','$final_sd','$final_vat','$total_product_price','$extra_instruction','$product_not_available')";
